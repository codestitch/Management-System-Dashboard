<?php
	...
	if (isset($_SESSION[MERCHANT_APPNAME.'_PORTAL_MEMBER_ID'])) {
		$session_checker = session_checker($_SESSION[MERCHANT_APPNAME.'_PORTAL_MEMBER_ID'], $_SESSION[MERCHANT_APPNAME.'_PORTAL_MEMBER_SESSION']);

		if ($session_checker == 'Expired') {
			unset($_SESSION[MERCHANT_APPNAME.'_PORTAL_MEMBER_ID']);
			unset($_SESSION[MERCHANT_APPNAME.'_PORTAL_MEMBER_EMAIL']);
			$_SESSION = array();
			session_unset();
			session_destroy();
	...
	if ($basename == 'login') {
		...
		if (($basename != '404') && ($basename != '500')) {
			if (!$_SESSION[MERCHANT_APPNAME.'_PORTAL_MEMBER_ID']) {
				unset($_SESSION[MERCHANT_APPNAME.'_PORTAL_MEMBER_ID']);
				unset($_SESSION[MERCHANT_APPNAME.'_PORTAL_MEMBER_EMAIL']);
				$_SESSION = array();
				session_unset();
				...
				$memberID = $_SESSION[MERCHANT_APPNAME.'_PORTAL_MEMBER_ID'];
				$email = $_SESSION[MERCHANT_APPNAME.'_PORTAL_MEMBER_EMAIL'];
			}
		}
	}
	...
	function session_checker($memberID, $my_session_id) {
		require_once('../../seafood/php/api/cipher/cipher.class.php');
		...
		$url = PATH."portal.php?function=".urlencode($function);
		$cURL = curl_init();
