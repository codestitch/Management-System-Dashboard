<?php
	...
	if (isset($_SESSION[MERCHANT_APPNAME.'_DASHBOARD_ACCOUNT_ID'])) {
		$session_checker = session_checker($_SESSION[MERCHANT_APPNAME.'_DASHBOARD_ACCOUNT_ID'], $_SESSION[MERCHANT_APPNAME.'_DASHBOARD_ACCOUNT_SESSION']);
	...
	if ($basename == 'login') {
		...
		if (($basename != '404') && ($basename != '500')) {
			if (!$_SESSION[MERCHANT_APPNAME.'_DASHBOARD_ACCOUNT_ID']) {
				...
				$accountID = $_SESSION[MERCHANT_APPNAME.'_DASHBOARD_ACCOUNT_ID'];
				$my_session_id = $_SESSION[MERCHANT_APPNAME.'_DASHBOARD_ACCOUNT_SESSION'];
			}
		}
	}
	...
	function session_checker($accountID, $my_session_id) {
		...
		$url = PATH."dashboard.php?function=".urlencode($function);
		$cURL = curl_init();
