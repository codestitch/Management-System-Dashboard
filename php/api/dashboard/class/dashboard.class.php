<?php
	...
	**/
	...
	class dashboard {
				...
				while ($row = $result->fetch_assoc()) {
					if (isset($row['result'])) {
						...
						if (strtolower($row['platform']) == 'android') {
							array_push($android_PUSH_ID, $row['pushID']);
						} elseif (strtolower($row['platform']) == 'ios') {
							array_push($ios_PUSH_ID, $row['pushID']);
						}
					}								
				}

				if (!$error_flag) {
					$gcm_chunk = array_chunk($android_PUSH_ID, 1000);

					for ($i=0; $i<count($gcm_chunk); $i++) {
						$push->android($gcm_chunk[$i], $message, $type);
					}

					$push->ios($ios_PUSH_ID, $message, CERTIFICATE, ENVIRONMENT, $type);
					$sql->close();
		...
		/********** Add Press **********/
		public function add_press($accountID, $my_session_id, $image, $status) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			if (!isset($image) || (!$image)) {
				$logs->write_logs('Dashboard Add Press', $file_name, "Bad Request - Invalid/Empty [image: $image]");
				die($error400);
				return;
			}

			if (!isset($status) || (!$status)) {
				$logs->write_logs('Dashboard Add Press', $file_name, "Bad Request - Invalid/Empty [status: $status]");
				die($error400);
				return;
			}
			...
			// Prepared statement, stage 1: prepare
			if (!($sql = $mysqli->prepare($qry))) {
				$logs->write_logs('MySQLi Prepare', $file_name, "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_add_press] [Query: $qry]");
			    die($error000);
				return;
			}

			if (!$sql->bind_param('ssss', $accountID, $my_session_id, $image, $status)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_add_press] [Param: $accountID, $my_session_id, $image, $status]");
			    die($error000);
				return;
			}
			...
			// Execute Prepared Statement
			if (!$sql->execute()) {
				$logs->write_logs('MySQLi Execute', $file_name, "Execute failed: (" . $sql->errno . ") " . $sql->error . "\t [Procedure: dashboard_add_press] [Query: $qry]");
			    die($error000);
				return;
			}
		...
		/********** Delete Press **********/
		public function delete_press($accountID, $my_session_id, $pressID) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			$error400 = self::$tmp_error400;
			$error1329 = self::$tmp_error1329;
			$qry = "CALL dashboard_delete_press(?, ?, ?)";

			if (!isset($accountID) || (!$accountID)) {
				$logs->write_logs('Dashboard Delete Press', $file_name, "Bad Request - Invalid/Empty [accountID: $accountID]");
				die($error400);
				return;
			}

			if (!isset($my_session_id) || (!$my_session_id)) {
				$logs->write_logs('Dashboard Delete Press', $file_name, "Bad Request - Invalid/Empty [my_session_id: $my_session_id]");
				die($error400);
				return;
			}

			if (!isset($pressID) || (!$pressID)) {
				$logs->write_logs('Dashboard Delete Press', $file_name, "Bad Request - Invalid/Empty [pressID: $pressID]");
				die($error400);
				return;
			}
			...
			// Prepared statement, stage 1: prepare
			if (!($sql = $mysqli->prepare($qry))) {
				$logs->write_logs('MySQLi Prepare', $file_name, "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_delete_press] [Query: $qry]");
			    die($error000);
				return;
			}

			if (!$sql->bind_param('sss', $accountID, $my_session_id, $pressID)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_delete_press] [Param: $accountID, $my_session_id, $pressID]");
			    die($error000);
				return;
			}
			...
			$accountID = filter_var($accountID, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH);
			$my_session_id = filter_var($my_session_id, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH);
			$pressID = filter_var($pressID, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH);

			// Execute Prepared Statement
			if (!$sql->execute()) {
				$logs->write_logs('MySQLi Execute', $file_name, "Execute failed: (" . $sql->errno . ") " . $sql->error . "\t [Procedure: dashboard_delete_press] [Query: $qry]");
			    die($error000);
				return;
			}
		...
		/********** Update Press **********/
		public function update_press($accountID, $my_session_id, $pressID, $image, $status) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			$error400 = self::$tmp_error400;
			$error1329 = self::$tmp_error1329;
			$qry = "CALL dashboard_update_press(?, ?, ?, ?, ?)";

			if (!isset($accountID) || (!$accountID)) {
				$logs->write_logs('Dashboard Update Press', $file_name, "Bad Request - Invalid/Empty [accountID: $accountID]");
				die($error400);
				return;
			}

			if (!isset($my_session_id) || (!$my_session_id)) {
				$logs->write_logs('Dashboard Update Press', $file_name, "Bad Request - Invalid/Empty [my_session_id: $my_session_id]");
				die($error400);
				return;
			}

			if (!isset($pressID) || (!$pressID)) {
				$logs->write_logs('Dashboard Update Press', $file_name, "Bad Request - Invalid/Empty [pressID: $pressID]");
				die($error400);
				return;
			}

			if (!isset($image) || (!$image)) {
				// $logs->write_logs('Dashboard Update Press', $file_name, "Bad Request - Invalid/Empty [image: $image]");
				// die($error400);
				// return;
				$image = NULL;
			}
			...
			if (!isset($status) || (!$status)) {
				$logs->write_logs('Dashboard Update Press', $file_name, "Bad Request - Invalid/Empty [status: $status]");
				die($error400);
				return;
			}
			...
			// Prepared statement, stage 1: prepare
			if (!($sql = $mysqli->prepare($qry))) {
				$logs->write_logs('MySQLi Prepare', $file_name, "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_update_press] [Query: $qry]");
			    die($error000);
				return;
			}

			if (!$sql->bind_param('sssss', $accountID, $my_session_id, $pressID, $image, $status)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_update_press] [Param: $accountID, $my_session_id, $pressID, $image, $status]");
			    die($error000);
				return;
			}
			...
			// Execute Prepared Statement
			if (!$sql->execute()) {
				$logs->write_logs('MySQLi Execute', $file_name, "Execute failed: (" . $sql->errno . ") " . $sql->error . "\t [Procedure: dashboard_update_press] [Query: $qry]");
			    die($error000);
				return;
			}
		...
		/********** Add Promo **********/
		public function add_promo($accountID, $my_session_id, $name, $image, $status) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			if (!$sql->bind_param('sssss', $accountID, $my_session_id, $name, $image, $status)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_add_promo] [Param: $accountID, $my_session_id, $name, $image, $status]");
			    die($error000);
				return;
			}
		...
		/********** Update Promo **********/
		public function update_promo($accountID, $my_session_id, $promoID, $name, $image, $status) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			if (!isset($image) || (!$image)) {
				// $logs->write_logs('Dashboard Update Promo', $file_name, "Bad Request - Invalid/Empty [image: $image]");
				// die($error400);
				// return;
				$image = NULL;
			}
			...
			if (!$sql->bind_param('ssssss', $accountID, $my_session_id, $promoID, $name, $image, $status)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_update_promo] [Param: $accountID, $my_session_id, $promoID, $name, $image, $status]");
			    die($error000);
				return;
			}
		...
		/********** Add Loyalty **********/
		public function add_loyalty($accountID, $my_session_id, $name, $points, $terms, $description, $status) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			if (!$sql->bind_param('sssisss', $accountID, $my_session_id, $name, $points, $terms, $description, $status)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_add_loyalty] [Param: $accountID, $my_session_id, $name, $points, $terms, $description, $status]");
			    die($error000);
				return;
			}
		...
		/********** Update Loyalty **********/
		public function update_loyalty($accountID, $my_session_id, $loyaltyID, $name, $points, $terms, $description, $status) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			if (!$sql->bind_param('ssssisss', $accountID, $my_session_id, $loyaltyID, $name, $points, $terms, $description, $status)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_update_loyalty] [Param: $accountID, $my_session_id, $loyaltyID, $name, $points, $terms, $description, $status]");
			    die($error000);
				return;
			}
		...
		/********** Add Staff **********/
		public function add_staff($accountID, $my_session_id, $name, $description, $image, $status) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			$error400 = self::$tmp_error400;
			$error1329 = self::$tmp_error1329;
			$qry = "CALL dashboard_add_staff(?, ?, ?, ?, ?, ?)";

			if (!isset($accountID) || (!$accountID)) {
				$logs->write_logs('Dashboard Add Staff', $file_name, "Bad Request - Invalid/Empty [accountID: $accountID]");
				die($error400);
				return;
			}

			if (!isset($my_session_id) || (!$my_session_id)) {
				$logs->write_logs('Dashboard Add Staff', $file_name, "Bad Request - Invalid/Empty [my_session_id: $my_session_id]");
				die($error400);
				return;
			}

			if (!isset($name) || (!$name)) {
				$logs->write_logs('Dashboard Add Staff', $file_name, "Bad Request - Invalid/Empty [name: $name]");
				die($error400);
				return;
			}

			if (!isset($description) || (!$description)) {
				$logs->write_logs('Dashboard Add Staff', $file_name, "Bad Request - Invalid/Empty [description: $description]");
				die($error400);
				return;
			}

			if (!isset($image) || (!$image)) {
				$logs->write_logs('Dashboard Add Staff', $file_name, "Bad Request - Invalid/Empty [image: $image]");
				die($error400);
				return;
			}

			if (!isset($status) || (!$status)) {
				$logs->write_logs('Dashboard Add Staff', $file_name, "Bad Request - Invalid/Empty [status: $status]");
				die($error400);
				return;
			}
			...
			// Prepared statement, stage 1: prepare
			if (!($sql = $mysqli->prepare($qry))) {
				$logs->write_logs('MySQLi Prepare', $file_name, "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_add_staff] [Query: $qry]");
			    die($error000);
				return;
			}

			if (!$sql->bind_param('ssssss', $accountID, $my_session_id, $name, $description, $image, $status)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_add_staff] [Param: $accountID, $my_session_id, $name, $description, $image, $status]");
			    die($error000);
				return;
			}
			...
			// Execute Prepared Statement
			if (!$sql->execute()) {
				$logs->write_logs('MySQLi Execute', $file_name, "Execute failed: (" . $sql->errno . ") " . $sql->error . "\t [Procedure: dashboard_add_staff] [Query: $qry]");
			    die($error000);
				return;
			}
		...
		/********** Delete Staff **********/
		public function delete_staff($accountID, $my_session_id, $staffID) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			$error400 = self::$tmp_error400;
			$error1329 = self::$tmp_error1329;
			$qry = "CALL dashboard_delete_staff(?, ?, ?)";

			if (!isset($accountID) || (!$accountID)) {
				$logs->write_logs('Dashboard Delete Staff', $file_name, "Bad Request - Invalid/Empty [accountID: $accountID]");
				die($error400);
				return;
			}

			if (!isset($my_session_id) || (!$my_session_id)) {
				$logs->write_logs('Dashboard Delete Staff', $file_name, "Bad Request - Invalid/Empty [my_session_id: $my_session_id]");
				die($error400);
				return;
			}

			if (!isset($staffID) || (!$staffID)) {
				$logs->write_logs('Dashboard Delete Staff', $file_name, "Bad Request - Invalid/Empty [staffID: $staffID]");
				die($error400);
				return;
			}
			...
			// Prepared statement, stage 1: prepare
			if (!($sql = $mysqli->prepare($qry))) {
				$logs->write_logs('MySQLi Prepare', $file_name, "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_delete_staff] [Query: $qry]");
			    die($error000);
				return;
			}

			if (!$sql->bind_param('sss', $accountID, $my_session_id, $staffID)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_delete_staff] [Param: $accountID, $my_session_id, $staffID]");
			    die($error000);
				return;
			}
			...
			$accountID = filter_var($accountID, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH);
			$my_session_id = filter_var($my_session_id, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH);
			$staffID = filter_var($staffID, FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH);

			// Execute Prepared Statement
			if (!$sql->execute()) {
				$logs->write_logs('MySQLi Execute', $file_name, "Execute failed: (" . $sql->errno . ") " . $sql->error . "\t [Procedure: dashboard_delete_staff] [Query: $qry]");
			    die($error000);
				return;
			}
		...
		/********** Update Staff **********/
		public function update_staff($accountID, $my_session_id, $staffID, $name, $description, $image, $status) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			$error400 = self::$tmp_error400;
			$error1329 = self::$tmp_error1329;
			$qry = "CALL dashboard_update_staff(?, ?, ?, ?, ?, ?, ?)";

			if (!isset($accountID) || (!$accountID)) {
				$logs->write_logs('Dashboard Update Staff', $file_name, "Bad Request - Invalid/Empty [accountID: $accountID]");
				die($error400);
				return;
			}

			if (!isset($my_session_id) || (!$my_session_id)) {
				$logs->write_logs('Dashboard Update Staff', $file_name, "Bad Request - Invalid/Empty [my_session_id: $my_session_id]");
				die($error400);
				return;
			}

			if (!isset($staffID) || (!$staffID)) {
				$logs->write_logs('Dashboard Update Staff', $file_staffID, "Bad Request - Invalid/Empty [staffID: $staffID]");
				die($error400);
				return;
			}

			if (!isset($name) || (!$name)) {
				$logs->write_logs('Dashboard Update Staff', $file_name, "Bad Request - Invalid/Empty [name: $name]");
				die($error400);
				return;
			}

			if (!isset($description) || (!$description)) {
				$logs->write_logs('Dashboard Update Staff', $file_name, "Bad Request - Invalid/Empty [description: $description]");
				die($error400);
				return;
			}

			if (!isset($image) || (!$image)) {
				// $logs->write_logs('Dashboard Update Staff', $file_name, "Bad Request - Invalid/Empty [image: $image]");
				// die($error400);
				// return;
				$image = NULL;
			}
			...
			if (!isset($status) || (!$status)) {
				$logs->write_logs('Dashboard Update Staff', $file_name, "Bad Request - Invalid/Empty [status: $status]");
				die($error400);
				return;
			}
			...
			// Prepared statement, stage 1: prepare
			if (!($sql = $mysqli->prepare($qry))) {
				$logs->write_logs('MySQLi Prepare', $file_name, "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_update_staff] [Query: $qry]");
			    die($error000);
				return;
			}

			if (!$sql->bind_param('sssssss', $accountID, $my_session_id, $staffID, $name, $description, $image, $status)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_update_staff] [Param: $accountID, $my_session_id, $staffID, $name, $description, $image, $status]");
			    die($error000);
				return;
			}
			...
			// Execute Prepared Statement
			if (!$sql->execute()) {
				$logs->write_logs('MySQLi Execute', $file_name, "Execute failed: (" . $sql->errno . ") " . $sql->error . "\t [Procedure: dashboard_update_staff] [Query: $qry]");
			    die($error000);
				return;
			}
		...
		/********** Add Location **********/
		public function add_location($accountID, $my_session_id, $name, $address, $latitude, $longitude, $branch, $phone, $email, $status, $loyalty, $image) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			if (!$sql->bind_param('ssssssssssss', $accountID, $my_session_id, $name, $address, $latitude, $longitude, $branch, $phone, $email, $status, $loyalty, $image)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_add_location] [Param: $accountID, $my_session_id, $name, $address, $latitude, $longitude, $branch, $phone, $email, $status, $loyalty, $image]");
			    die($error000);
				return;
			}
		...
		/********** Update Location **********/
		public function update_location($accountID, $my_session_id, $locID, $name, $address, $latitude, $longitude, $branch, $phone, $email, $status, $loyalty, $image) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			if (!$sql->bind_param('sssssssssssss', $accountID, $my_session_id, $locID, $name, $address, $latitude, $longitude, $branch, $phone, $email, $status, $loyalty, $image)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_update_location] [Param: $accountID, $my_session_id, $locID, $name, $address, $latitude, $longitude, $branch, $phone, $email, $status, $loyalty, $image]");
			    die($error000);
				return;
			}
		...
		public function add_product($accountID, $my_session_id, $category, $name, $description, $price, $image, $status) {
			...
			if (!isset($price) ) {
				$logs->write_logs('Dashboard Add Product', $file_name, "Bad Request - Invalid/Empty [price: $price]");
				die($error400);
				return;
			}
		...
		/********** Delete Product **********/
		public function delete_product($accountID, $my_session_id, $prodID) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			$error400 = self::$tmp_error400;
			$error1329 = self::$tmp_error1329;
			$qry = "CALL dashboard_delete_product(?, ?, ?)";

			if (!isset($accountID) || (!$accountID)) {
				$logs->write_logs('Dashboard Delete Product', $file_name, "Bad Request - Invalid/Empty [accountID: $accountID]");
				die($error400);
				return;
			}

			if (!isset($my_session_id) || (!$my_session_id)) {
				$logs->write_logs('Dashboard Delete Product', $file_name, "Bad Request - Invalid/Empty [my_session_id: $my_session_id]");
				die($error400);
				return;
			}

			if (!isset($prodID) || (!$prodID)) {
				$logs->write_logs('Dashboard Delete Product', $file_name, "Bad Request - Invalid/Empty [prodID: $prodID]");
				die($error400);
				return;
			}
			...
			// Prepared statement, stage 1: prepare
			if (!($sql = $mysqli->prepare($qry))) {
				$logs->write_logs('MySQLi Prepare', $file_name, "Prepare failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_delete_product] [Query: $qry]");
			    die($error000);
				return;
			}

			if (!$sql->bind_param('sss', $accountID, $my_session_id, $prodID)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_delete_product] [Param: $accountID, $my_session_id, $prodID]");
			    die($error000);
				return;
			}
			...
			// Execute Prepared Statement
			if (!$sql->execute()) {
				$logs->write_logs('MySQLi Execute', $file_name, "Execute failed: (" . $sql->errno . ") " . $sql->error . "\t [Procedure: dashboard_delete_product] [Query: $qry]");
			    die($error000);
				return;
			}
		...
		public function view_record($accountID, $my_session_id, $table, $recordID) {
			...
			if ($result->num_rows > 0) {
				...
				if (!isset($row['result'])) {
					...
					return json_encode(array(array("response"=>html_entity_decode($row['result'], ENT_QUOTES, 'UTF-8') ) ) );
				}
			} else {
				return json_encode(array(array("response"=>"Error")));
			}
		...
		public function add_sku($accountID, $my_session_id, $name, $skuCode, $price, $promoType, $points, $description, $status) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			if (!$sql->bind_param('ssssisiss', $accountID, $my_session_id, $name, $skuCode, $price, $promoType, $points, $description, $status)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_add_sku] [Param: $accountID, $my_session_id, $name, $skuCode, $price, $promoType, $points, $description, $status]");
			    die($error000);
				return;
			}
		...
		/********** Update SKU **********/
		public function update_sku($accountID, $my_session_id, $skuID, $name, $skuCode, $price, $points, $description, $status) {
			include("php/api/push/push.class.php");
			$push = new push_notification;
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			...
			if (!$sql->bind_param('sssssiiss', $accountID, $my_session_id, $skuID, $name, $skuCode, $price, $points, $description, $status)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: dashboard_update_sku] [Param: $accountID, $my_session_id, $skuID, $name, $skuCode, $price, $points, $description, $status]");
			    die($error000);
				return;
			}
		...
		public function unlink_image($source) {	
			$source = str_replace("http://seafood.appsolutely.ph/", "", $source);
