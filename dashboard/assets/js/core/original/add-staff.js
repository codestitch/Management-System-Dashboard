...
function UploadImage() {
    ...
    $('#upload_press_form').ajaxSubmit({
        url: 'php/upload.php',
        data: { file_name: 'press_image', purpose: 'staff' },
        type: 'post',
        ...
        success: function(result) { 
            ...
            hide_loading("#contentBody");
                ...
                var staff = $("#staffField").val(),
                    status = ($("#statusField").is(':checked') == true) ? "active" : "inactive",
                    description = $( "#descriptionField").code();

                if (!staff || staff == null || staff == "") {
                    toastr['warning']("Kindly fill staff title", "Invalid Entry");
            hide_loading("#contentBody");
                    return;
                }
                else if (!description || description == null || description == "" || description == "<p><br></p>" || description == "<br>") {
                    toastr['warning']("Kindly fill staff description", "Invalid Entry");
            hide_loading("#contentBody");
                    return;
                }

                // upload image
                SendData(staff, description, status, result);
...
function SendData (name, description, status, image) {

	var _post = "function=add_staff&name="+name+"&description="+description+"&status="+status+"&image="+image;
